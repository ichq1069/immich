...
if [ "$CHOICE" = "1" ]; then
  ...
  # 检查端口是否被占用
  if ss -tlnp 2>/dev/null | grep -q ":$CONFIG_PORT "; then
    EXISTING_PID=$(ss -tlnp 2>/dev/null | grep ":$CONFIG_PORT " | grep -oP 'pid=\K[0-9]+' | head -1)
    echo -e "${YELLOW}端口 $CONFIG_PORT 已被进程 ${EXISTING_PID:-unknown} 占用${NC}"
    read -p "是否关闭该进程? (y/N): " KILL_CHOICE
    if [ "$KILL_CHOICE" = "y" ] || [ "$KILL_CHOICE" = "Y" ]; then
      [ -n "$EXISTING_PID" ] && kill $EXISTING_PID 2>/dev/null && echo -e "${GREEN}已关闭进程 $EXISTING_PID${NC}" || echo -e "${RED}关闭失败，请手动处理${NC}"
      sleep 1
    else
